...
open PYOUT, ">$path/pie_inc.py" or die $!;
print OUT "Post translational modifications (PTM) data analysis summary:\n\n";
print OUT "Citing PTM-Pro:\n
A). Please add to your manuscript the following sentence (typically in the \"Methods\" section): \"High-confidence PTMs were profiled using PTM-Pro tool[1]\". 
B). In oral presentations, include attribution to \"PTM-Pro, www.ptm-pro.inhouseprotocols.com\" on pages that include information to PTM-Pro.
[1]. Patil, A. H., Datta, K. K., et al., Dissecting Candida pathobiology: Post-translational modifications on the Candida tropicalis proteome. 2018. OMICS: A Journal of Integrative Biology. 22, 544-552. [PMID: 30106353].\n
";
print OUT "SUMMARY:\n";
...
while (($ttmod, $sumMods) = each %TotalUniqueModifications)
{
	...
	foreach $aminoAcids (A..Z)
	{
		...
		if (exists $IndividualUniqueModifications{$temp_var})
		{
			...
			$uniqSites{$ttmod} .= "$aminoAcids\:$IndividualUniqueModifications{$temp_var}\;";
		}
	}
	$serial_nu++;
}
...
#$uniqSites{$ttmod} .= "$aminoAcids\:$IndividualUniqueModifications{$temp_var}\;";
$sizeInitializeModfn=0;
while (($uSitesKey, $uSitesVal) = each %uniqSites)
{
	@EachModf = split (/\;/, $uSitesVal);
	$modfnListSize = scalar @EachModf;
	if ($modfnListSize > $sizeInitializeModfn)
	{
		$sizeInitializeModfn = $modfnListSize;
	}
}
while (($uSitesKey, $uSitesVal) = each %uniqSites)
{
	@EachModf = split (/\;/, $uSitesVal);
	$modfnListSize = scalar @EachModf;
#	print PYOUT "\#$uSitesKey\t$modfnListSize - $sizeInitializeModfn\n";
	if ($modfnListSize < $sizeInitializeModfn)
	{
		$diffOfElements = $sizeInitializeModfn - $modfnListSize;
		#print PYOUT "\#Difference = $diffOfElements\n";
		for ($id =0; $id <$diffOfElements; $id++)
		{
			#$sizeInitializeModfn = $modfnListSize;
			$uniqSites{$uSitesKey} .= "Zero:0;";
		#	print PYOUT "\# $uSitesKey = Zero:0;\n";
		}
	}
#	print PYOUT "Value: #$uniqSites{$uSitesKey}\n";
}
while (($uSitesKey, $uSitesVal) = each %uniqSites)
{
	$modList .= "\"$uSitesKey\"\,";
	@sitesModList = split (/\;/, $uSitesVal);
	foreach $ze (@sitesModList)
	{
		($sKeySite,$sValSite) = split (/\:/,$ze);
		if ($ze =~ /Zero/) {$ze =~ s/Zero://;}
		$pyUnqSites .= "\"$ze\"\,";
		$pyUnqValList .= "$sValSite\,";	
	}
	$pyUnqValList =~ s/,$//;
	$pyPreValList .= "\[$pyUnqValList\],";
	#print PYOUT "\#-> $pyUnqValList\n";
	#print PYOUT "\#-> $pyPreValList\n";
	$pyUnqValList = "";
}
$modList =~ s/,$//;
$pyUnqSites =~ s/,$//;
...
print PYOUT "ptms = \[$modList\]\n";
print PYOUT "unique_sites = \[$pyUnqSites\]\n";
print PYOUT "val_list = \[$pyPreValList\]\n";
