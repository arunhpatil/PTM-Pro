#!/usr/bin/perl
# USAGE: perl psp.pl /tmp/5cf8aedb2fd10/Input_tryp_AP_PTM_prtSite_ptmWindow.txt "Homo sapiens" output_4test.txt 
$input_file = $ARGV[0];
$organism = $ARGV[1];
$output_file = $ARGV[2];
$organism =~ s/\#/ /g;
...
open (PSP, "dbPTM.txt") or die $!;
#open (PSP, "PSP_datasets.txt") or die $!;
#open (PSP, "$organism") or die $!;
#Acetylation     GRF1    P42643  Arabidopsis thaliana    ______MATPGASSA
while (<PSP>)
{
	chomp ($_);
	$_ =~ s/\r//g;
	@lines = split (/\t/, $_);
	if ($lines[3] eq "$organism")
	{
		$psp_gene = $lines[1];
		$psp_modif = $lines[0];
		$psp_win = $lines[4];
		$psp_win =~ tr/[a-z]/[A-Z]/;
		#print "-->$psp_gene<--\*-->$psp_modif<--\*-->$psp_win<---\n";
